...
process_one_file <- function(infile_directory, filename) {
    ...
    fitted = tryCatch({
        lm(adjusted_yield ~ as.factor(currency) + as.factor(firm), df) 
        # lm(adjusted_yield ~ as.factor(currency)*duration_diff+ as.factor(firm), df) 
    }, warning = function(w) {
    ...
    if ( is.null(fitted)) {
        error_result = filename
        error_result = paste0(error_result, ",")
   ...
   for (curr_index in 1:9) {
    ...
    max_firm = max(abs(copy[0:copy_index]))
    line = paste0(line, ",", max_firm)

   ## # each currency (except USD)
   ## for (curr_index in 1:9) {
   ##     str_index = paste0("as.factor(currency)", curr_index, ":duration_diff")
   ##     value = tryCatch({
   ##     	     toString( coeffs[str_index, 1] )
   ##      }, warning = function(w) {
   ##           return("")
...
header = paste0(header, ",max_abs_firm") 
## for (curr_index in 1:9) {
##     header = paste0(header, ",", currencies_except_USD[curr_index], "_duration")
## }
