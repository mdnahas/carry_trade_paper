...
AUD & .35 & .28 \\
CAD & .27 & .57 \\
all currencies & .71 & .81 \\  % Liao value from the bottom of page 2.
\hline
\end{tabular}
...
\begin{figure}[H]  % h = here
\textbf{\caption{\label{Liao_XY} CIP basis vs. credit spread differential}}
\noindent \textbf{Panel A:} \newline
...
\newline 
 Note: {\small Plots of CIP basis against RCSD.  These plots have the 6 currencies used by Liao for Liao's time period, January 1998 to July 2016.  Both CIP basis and RCSD were calculated using swap rates, and CIP basis with a 5-year horizon.  Panel A is from this work and Panel B is from Liao's.  The red-dashed trend line in Panel A is from a linear regression.  I did not find anything in Liao documenting what the black line was in his plot.  In Panel A, the extreme points in the lower right are from JPY from mid-2009 to mid-2010. }
\newline \noindent Source: (Panel B only) Liao(2016)\cite{Liao2016}
\end{figure}
...
\begin{figure}[H]  % h = here
\textbf{\caption{\label{Liao_XY_all} CIP basis vs. credit spread differential}}
\includegraphics[height=3.1in]{images/Liao2016Figure5_xcbs_alldata}
\newline 
 Note: {\small CIP basis against RCSD, using data from this work.  This plot has all 9 currencies, for the entire time period January 1998 to March 2017.  The extreme points in the lower right are from JPY from mid-2009 to mid-2010. }
\newline \noindent Source: (Panel B only) Liao(2016)\cite{Liao2016}
\end{figure}
...
The goal of replacing swap rates with government rates was to obtain greater accuracy, not to improve the correlation.  But the correlations of the pooled sample did improve from 71\% to 85\%.   Table~\ref{govt_correl_table} has separate correlations for each currency.  
...
An F test was done on the data using government rates, with constraints $\kappa_1=1$ and $\kappa_o=0$.  The na\"ive application of the F test has an F statistic of .9933 and a p-value of .8884.  As with swap rates, consecutive sample errors were correlated.  When adjusted for 1/18th the sample size, the p-value is .9744.   The model is clearly not rejected.  This p-value is higher than the result using swap rates, .2387.  The model fits better using government rates.  
...
The model in equation~\eqref{first_model} assumes that firms issue at most one bond per currency, but in reality, there may be many.  In this subsection, the single-bond model is modified.  The important change is that errors are allowed to be correlated for bonds that share the same currency and firm.  
