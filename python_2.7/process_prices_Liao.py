...
if len(sys.argv) != 4:
    raise Exception("bad number of arguments.  Saw " + str(len(sys.argv)) + " expected 4.  Program raw_data_dir input_directory output_directory")
...
swap_rates.load_files(raw_data_directory, "")
for filename in os.listdir(infile_directory):
    ...
    with open(os.path.join(infile_directory, filename), 'rb') as infile:
        ...
        for row in csv_reader:
            ...
            if math.isnan(swap_rates.get_swap_rate(currency, date, duration)):
                continue
            
            if firm not in firm_count:
                firm_count[ firm ] = 0
            firm_count[ firm ] += 1
    ...
    with open(os.path.join(infile_directory,filename), 'rb') as infile:
        ...
        with open(os.path.join(outfile_directory,filename), 'wb') as outfile:
            ...
            # copy header
            first_line = csv_reader.next()
            first_line.extend([ "swap_rate", "credit_spread", "combined_rating", "rating_bucket", "duration_bucket" ])
            ...
            for row in csv_reader:
                ...
                swap_rate = swap_rates.get_swap_rate(currency, date, duration)
                if math.isnan(swap_rate):
                    print("Skipping " + isin + " on " + str(date) + " because of missing swap rate", file=sys.stderr)
                    continue
                ...
                row.extend([ str(swap_rate),
                             str(yyield - swap_rate),
                             str(combined_rating),
